...
public enum ErrorCode {
    ...
    RESERVATION_NOT_FOUND("R001", "예약 정보가 없습니다.", HttpStatus.NOT_FOUND),
    ...
    DUPLICATE_RESERVATION("R003", "이미 해당 날짜와 시간에 예약이 존재합니다.", HttpStatus.CONFLICT),
    ...
    REFUND_POLICY_VIOLATION("R007", "환불 가능 기간이 지나 취소할 수 없습니다.", HttpStatus.BAD_REQUEST),
    ALREADY_CANCELED("R008", "이미 취소된 예약입니다.", HttpStatus.CONFLICT),
    UNAUTHORIZED_REVIEW_ACCESS("RV002", "본인이 작성한 리뷰에만 접근할 수 있습니다.", HttpStatus.FORBIDDEN),
    DUPLICATE_REVIEW("RV003", "이미 해당 예약에 대한 리뷰가 존재합니다.", HttpStatus.CONFLICT),
    DUPLICATE_USER_EMAIL("U002", "이미 존재하는 이메일 입니다.", HttpStatus.CONFLICT),
    ...
    REFRESH_TOKEN_NOT_FOUND("AUTH404", "리프레시 토큰을 찾을 수 없습니다.", HttpStatus.UNAUTHORIZED),
    MISMATCHED_REFRESH_TOKEN("AUTH405", "요청 정보와 리프레시 토큰 정보가 일치하지 않습니다.", HttpStatus.UNAUTHORIZED);
