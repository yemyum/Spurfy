...
""";
    ...
    private static String jsonContractBlock(String introMessage, boolean fallbackTone) {
        ...
        b.append("[응답 규칙] 반드시 순수 JSON 객체만 응답할 것. 마크다운 코드 블록(```), 백틱(`), 설명, 주석, 자연어는 절대 포함하지 말 것.\n");
        ...
        b.append("※ 'spaDescription' 필드는 문자열 배열 형태(List)이며, 정확히 2개의 설명 항목을 포함해야 함. 각 항목은 \"-\" 기호로 시작하는 한 줄짜리 설명이어야 함.\n");
        b.append("※ 'spaName'은 이모지 포함 + **굵게** + \"를 추천해요!\" 문장 포함.\n");
        b.append("{\n");
        b.append("  \"intro\": \"").append(introMessage).append("\",\n");
        b.append("  \"compliment\": \"이 견종의 성격, 분위기, 보호자에게 어필할만한 특징을 2줄로 요약\\n\\n\",\n");
        b.append("  \"recommendationHeader\": \"이런 아이에게는\\n\",\n");
        b.append("  \"spaName\": \"**스파 이름(이모지 포함)**를 추천해요!\\n\\n\",\n");
        ...
        // 클로징 후보 3개(두 버전 공통)
        b.append("마지막 줄(closing 필드)은 아래 3가지 중 하나를 정확히 선택해 그대로 출력:\n");
        b.append("- 우리 아이와 함께하는 스파 시간, 스퍼피가 기다리고 있을게요. 💙\n");
        b.append("- 소중한 반려견과 함께, 오늘은 특별한 스파 데이를 즐겨보세요. 💙\n");
    ...
    private static String buildIntroForLabels(boolean hasUserBreed, String userBreed,
        ...
        if (hasUserBreed) {
            ...
            return "정확한 견종을 찾지 못했지만,\\n걱정 마세요! 저 스피가 최적의 스파를 추천해드릴게요. 🤩\\n\\n";
        } else {
            return "제공해주신 정보들을 참고해서\\n반려견에게 알맞은 스파를 추천해드릴게요. 😉\\n\\n";
        }
    }
